...
import org.openflexo.module.oneway.controller.BPMNPerspective;
...
public class BPMNDiagramModuleView extends FMLControlledDiagramModuleView {

	@SuppressWarnings("unused")
	private static final Logger logger = Logger.getLogger(BPMNDiagramModuleView.class.getPackage().getName());

	private DGListener dgListener;
	private DSListener dsListener;

	public BPMNDiagramModuleView(FMLControlledDiagramEditor editor, BPMNPerspective perspective) {
		super(editor, perspective);
		dgListener = new DGListener(perspective.getProcessBrowser().getFIBBrowserWidget().getTechnologyComponent().getJTree());
		dsListener = new DSListener();
	}

	@Override
	public BPMNPerspective getPerspective() {
		return (BPMNPerspective) super.getPerspective();
	}

	@Override
	public String getInfoMessage() {
		return "BPMN modelling - CTRL-drag to draw edges";
	}
	...
	public void willHide() {
		...
		getPerspective().hideProcessBrowser();
	}
	...
	public void willShow() {
		...
		FlexoConceptInstance process;
		try {
			process = fmlControlledDiagram.execute("process");
			getPerspective().showProcessBrowser(process);
			// Activate drag&drop between browser and diagram
			getPerspective().getProcessBrowser().getFIBBrowserWidget().registerDragGestureListener(dgListener);
