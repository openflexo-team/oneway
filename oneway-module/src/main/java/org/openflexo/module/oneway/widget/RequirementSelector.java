...
/**
 * Widget allowing to select a Requirement
 * 
 * @author sguerin
 * 
 */
@SuppressWarnings("serial")
public class RequirementSelector extends FIBFlexoConceptInstanceSelector {

	static final Logger logger = Logger.getLogger(RequirementSelector.class.getPackage().getName());

	public static Resource FIB_FILE = ResourceLocator.locateResource("Fib/RequirementSelector.fib");

	public RequirementSelector(FlexoConceptInstance editedObject) {
		super(editedObject);
	}
	...
	public Type getExpectedType() {
		if (getFlexoController() != null && getFlexoController().getOnewayNature() != null) {
			System.out.println("Requirement concept = " + getFlexoController().getOnewayNature().getElementConcept());
			return getFlexoController().getOnewayNature().getRequirementConcept().getInstanceType();
		}
